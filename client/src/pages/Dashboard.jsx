...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
      try {
        const [campaignRes, userRes] = await Promise.all([
          fetch(`http://localhost:5000/api/campaigns/${scholarId}`),
  ...
  const handleDeleteQuest = async (e, questName) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/campaigns/${activeCampaign._id}/quests/${encodeURIComponent(questName)}`, {
        method: 'DELETE',
      });
